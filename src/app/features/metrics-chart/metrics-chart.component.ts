...
export class MetricsChartComponent implements OnChanges, AfterViewInit, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['device'] && this.device) {
      ...
      if (
        !this.selectedIface ||
        !this.device.interfaces.find((i) => i.name === this.selectedIface)
      ) {
  ...
  private renderChart(): void {
    ...
    if (!this.svg) {
      this.svg = d3.select(wrap).append('svg').attr('width', W).attr('height', H);
    } else {
    ...
    const yMax = d3.max(series, (d) => Math.max((d as any)[key1] ?? 0, (d as any)[key2] ?? 0)) ?? 1;
    ...
    g.append('g')
      ...
      .call((gr) => gr.selectAll('.tick line').attr('stroke', 'rgba(255,255,255,0.05)'));
    ...
    g.append('path')
      .datum(series)
      .attr('class', 'area-recv')
      .attr('d', area1)
      .attr('fill', 'rgba(0,212,170,0.08)');
    g.append('path')
      .datum(series)
      .attr('class', 'line-recv')
      .attr('d', line1)
      .attr('fill', 'none')
      .attr('stroke', '#00d4aa')
    ...
    g.append('path')
      .datum(series)
      .attr('class', 'area-sent')
      .attr('d', area2)
      .attr('fill', 'rgba(124,107,255,0.08)');
    g.append('path')
      .datum(series)
      .attr('class', 'line-sent')
      .attr('d', line2)
      .attr('fill', 'none')
      .attr('stroke', '#7c6bff')
      .attr('stroke-width', 1.5);
  ...
  private getSeriesKeys(): { key1: string; key2: string; label1: string; label2: string } {
    switch (this.selectedMetric) {
      ...
      case 'packets':
        return {
          key1: 'packetsRecvRate',
          key2: 'packetsSentRate',
          label1: 'PKT-RX',
          label2: 'PKT-TX',
        };
      case 'errors':
  ...
  onMetricChange(): void {} // triggers computed re-evaluation via signal reads in template
  onIfaceChange(): void {} // triggers computed re-evaluation via signal reads in template
